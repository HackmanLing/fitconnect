...
const LoginScreen = () => {
  ...
  const onLogin = async () => {
    ...
    ApiClient.post("/userRouter/Login", {email, password}).then(function (response) {
      let data = response.data
      if (data.status === 'success') {
        saveData("userDetails", data.data)
        saveData("session", data.sessionID)
      ...
      <ScrollView style={{ padding: "5%", paddingTop: StatusBar.currentHeight }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ marginTop: "5%", margin: 10 }}>
          ...
          <View style={{ marginBottom: '3%' }}>
            <TouchableOpacity activeOpacity={0.5} style={{ backgroundColor: '#292b2c', borderRadius: 10 }} onPress={onLogin}>
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: "center", width: '100%', paddingVertical: '5%', }}>
                <Text style={{ paddingTop: 5, color: "white", textAlign: 'center', fontFamily: 'Bold' }}>Login</Text>
              </View>
            </TouchableOpacity>
          </View>
          <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginBottom: '10%' }}>
            ...
            <TouchableOpacity activeOpacity={0.5} styles={{ margin: 0, padding: 0 }} onPress={() => navigation.navigate('Register')}>
